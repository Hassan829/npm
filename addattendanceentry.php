...
<!-- Modal -->
        ...
        <!--        form div-->
                <div class="col-md-12">
                <form action="addattendanceentry.php" method="post" class="form form-inline" onsubmit="return tableValidation();" name="attendanceForm">    
             ...
             $newDate = $newDate[0]."-".$newDate[1]."-".$newDate[2];
               ...
               while($row = mysqli_fetch_assoc($result )) {
                ...
                $employee_name = getemployeename($employee_id);  
                 ...
                 $empSalary = get_Employee_Salary_Data($employee_id);
                 $empSalary = mysqli_fetch_assoc($empSalary);       
                 if($empSalary['termination_date']==0 && $empSalary['termination_date'] ==0){
                        $addAttendance = true;
                       // echo "Termination date is 0 <br> ".$empSalary['emp_id'];
                    }elseif($newDate < $empSalary['termination_date']){
                        $addAttendance =true;
                       // echo "Termination date is  greater then 0 <br>".$empSalary['emp_id'];
                    }

                    /*if(($year >= $joinDate[0] && $month >= $joinDate[1]) && $generatePayroll
                        )*/
                    if($newDate>=$empSalary['join_date'] && $addAttendance){
        ...
        //                  var time_in_str = time_in.split(":");
                ...
                <?php } else{
                //  echo "Error    <br>".$empSalary['join_date'];
                }
                }?>  
...
function tableValidation(){
  ...
  var p_ids = document.forms['attendanceForm'].elements["employee_id[]"];
  for (var i = 0, len = p_ids.length; i < len; i++) {
    //alert(p_ids[i].value);

    var e = document.getElementById("status_option"+p_ids[i].value);
    var option =  e.options[e.selectedIndex].value;
    if(option === ""){
      alert("Please fill attendance for all personnel");
      document.getElementById("status_option"+p_ids[i].value).focus();
      return false;
    }
    var e = document.getElementById("site_location"+p_ids[i].value);
    var option =  e.options[e.selectedIndex].value;
    if(option === ""){
      alert("Please fill attendance for all personnel");
      document.getElementById("site_location"+p_ids[i].value).focus();
      return false;
    }
    var e = document.getElementById("remarks_option"+p_ids[i].value);
    var option =  e.options[e.selectedIndex].value;
    if(option === ""){
      alert("Please fill attendance for all personnel");
      document.getElementById("remarks_option"+p_ids[i].value).focus();
      return false;
    }
  }

...
//get employee all data with employee internal id
function getemployeedataWithInternalId($employeeInternalId){
    
    
     $conn = getconnection();
    
     $query = "SELECT employee_id, employee_internal_id, employee_NAME, employee_cnic, employee_nationality ,employee_email, employee_dob, employee_picture, employee_mobile, employee_address, employee_gender, employee_password, employee_username, employee_job_title, employee_role, employee_adder_id FROM employee WHERE employee_internal_id = {$employeeInternalId}";
...
function get_Employee_Salary_Data($id){
     ...
     $query = "SELECT account_no, iban_no, project_allowance, housing_allowance, conveyance_allowance, other_allowance, basic_salary, wps_salary, deductions, join_date, termination_date,emp_id FROM employee_salary WHERE emp_id = {$id}";
...
function InsertArrears($empid, $accountNo, $ibanNo, $projectAllowance, $housingAllowance, $conveyanceAllowance, $otherAllowance, $basicSalary, $wpsSalary, $totalPreviousPayments, $arrears, $grossSalary, $month, $year, $operator){
...
function getEmployeeArrears($employee_internal_id,$month,$year){
    $conn = getconnection();
     $query = "SELECT e.employee_id,pr.* FROM employee e join payroll pr on pr.emp_id = e.employee_id WHERE e.employee_internal_id = $employee_internal_id and pr.month = '$month' and pr.year = '$year' and pr.status = 'active'";
    $execute = mysqli_query($conn,$query);
    $result = array();
    if(!$execute){
        die("Error in query:". mysqli_error($conn));
...
function updateUserPassword($id,$password){
    ...
    $conn = getconnection();
//    = mysqli_connect('localhost','tomeship','root','tomeship');
      $query = "UPDATE employee SET employee_password='$password' WHERE employee_id='$id'";
    $execute = mysqli_query($conn,$query);
...
function updateArrears($id, $arrears){
    
    $conn = getconnection();

    $queryUpdate = "UPDATE payroll SET gross_salary = gross_salary+'$arrears' , arrears =  '$arrears' WHERE id = '$id'";
